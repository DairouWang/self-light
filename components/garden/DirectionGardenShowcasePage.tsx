...
export function DirectionGardenShowcasePage() {
  const { tiles } = useGardenState();
  const [selectedTileId, setSelectedTileId] = useState<string | null>(null);
  const directionTiles = tiles.filter((tile) => tile.zone === "direction");
  const selectedTile =
    directionTiles.find((tile) => tile.id === selectedTileId) ?? null;
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-[-8%] top-[-10%] h-[34rem] w-[34rem] rounded-full bg-[#eef0df]/84 blur-3xl" />
        <div className="absolute right-[-4%] top-[10%] h-[28rem] w-[28rem] rounded-full bg-[#d7ddc9]/64 blur-3xl" />
        <div className="absolute bottom-[-10%] left-[22%] h-[24rem] w-[24rem] rounded-full bg-[#c6cfb3]/54 blur-3xl" />
      </div>
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[94rem] flex-col justify-center gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_19rem]">
          <motion.section
            ...
            data-direction-showcase
            style={{ aspectRatio: `${directionGardenImageAspectRatio}` }}
          ...
          <aside className="rounded-[34px] border border-white/45 bg-white/56 p-5 text-[#5a4c40] shadow-[0_18px_40px_rgba(92,77,63,0.12)] backdrop-blur-lg">
            ...
            <p className="mt-3 font-display text-[1.45rem] leading-none">
              A focused view of the Direction zone.
            </p>
            <p className="mt-4 text-sm leading-relaxed text-[#6e6054]">
              The standalone Direction Garden uses the supplied transparent base
              and one shared stone-brick asset. The brick path stays fixed in
              one layered canvas so the route reads clearly as it grows.
            </p>
