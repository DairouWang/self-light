...
export function EmotionGardenShowcasePage() {
  const { tiles } = useGardenState();
  const [selectedTileId, setSelectedTileId] = useState<string | null>(null);
  const emotionTiles = tiles.filter((tile) => tile.zone === "emotion");
  const selectedTile =
    emotionTiles.find((tile) => tile.id === selectedTileId) ?? null;
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[94rem] flex-col justify-center gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_19rem]">
          <motion.section
            ...
            data-emotion-showcase
            style={{ aspectRatio: "2914 / 1440" }}
          ...
          <aside className="rounded-[34px] border border-white/45 bg-white/56 p-5 text-[#5a4c40] shadow-[0_18px_40px_rgba(92,77,63,0.12)] backdrop-blur-lg">
            ...
            <p className="mt-3 font-display text-[1.45rem] leading-none">
              A focused view of the Emotion zone.
            </p>
            <p className="mt-4 text-sm leading-relaxed text-[#6e6054]">
              The standalone Emotion Garden reuses the shared route state and a
              dedicated layered canvas. The decorative base stays separate from
              the interactive stone tiles.
            </p>
